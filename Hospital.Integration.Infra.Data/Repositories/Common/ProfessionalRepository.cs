...
public class ProfessionalRepository : IProfessionalRepository
{
    ...
    public ProfessionalRepository(IUnitOfWork unitOfWork) =>
        _unitOfWork = unitOfWork;

    public async Task<int> GetByParamnsCountAsync(ProfessionalsQuery professionalquery)
    {
        var (query, paramFilter) = MountFilter(ProfessionalStmt.SelectByParamnsCount, professionalquery);
    ...
    public async Task<IEnumerable<Professional>> GetByParamnsAsync(ProfessionalsQuery professionalquery)
    {
        var (query, paramFilter) = MountFilter(ProfessionalStmt.SelectByParamns, professionalquery, professionalquery.FilterPaging);

        return await _unitOfWork.Connection.QueryAsync<Professional>(
            sql: query,
            param: paramFilter,
            commandTimeout: MaxTimeOut,
            commandType: CommandType.Text);
    ...
    private static (string, DynamicParameters) MountFilter(string select, ProfessionalsQuery filter, FilterPaging? filterPaging = null)
    {
